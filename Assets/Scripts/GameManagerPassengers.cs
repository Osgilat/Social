...
public class GameManagerPassengers : NetworkBehaviour {
    ...
    void Initialization()
    {
        ...
        { 
            player.GetComponent<UIManager>().CmdHybernate(player);
        }
        /*
       ...
       // m_MessageText.text = "SESSION_ID: " + GameObject.FindGameObjectWithTag("MainCamera").GetComponent<SceneInfo>().sessionID;
        ...
        void Update()
	{
        ...
        {
			GameManagerPassengers.escaped = true;
		}
    ...
    private IEnumerator RoundEnding (){
        ...
        for (int i = 0; i < players.Length; i++) {
            ...
            if (players[i].GetComponent<HybernationSystem>().hybernationSphere.GetComponent<MeshRenderer>().enabled)
            {
            ...
            players[i].GetComponent<UIManager>().RespawnPlayers(players[i], spawnPoints[i].transform.position, spawnPoints[i].transform.rotation, true);
        ...
        damagedBot.GetComponent<UIManager>().Cmd_TransformPlayer(damagedBot, new Vector3(0, 0, 0), Quaternion.identity, true);
